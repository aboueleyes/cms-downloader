...
from src.constants import BACK, COURSE_REGEX, COURSE_REPLACE, HOST
...
def authenticate_user(username, password):
    ...
    request_session = session.get(
        HOST, verify=False, auth=HttpNtlmAuth(username, password)
    )
...
def get_course_soup(course_url, username, password, session):
    ...
    course_page = session.get(
        course_url, verify=False, auth=HttpNtlmAuth(username, password)
    )
...
def get_announcements(course_page_soup):
    ...
    announcements = course_page_soup.find("div", class_="row").find_all(
        lambda tag: (tag.name in ["b"] and not tag.find("b")) or (tag.name in ["p"])
    )
...
def print_announcement(course, username, password, course_url, session, console):
    ...
    announcements = get_announcements(
        get_course_soup(course_url, username, password, session)
    )
...
def get_downloaded_items(course):
    ...
    names = [
        os.listdir(f"{DOWNLOADS_DIR}/{course}/{directory}")
